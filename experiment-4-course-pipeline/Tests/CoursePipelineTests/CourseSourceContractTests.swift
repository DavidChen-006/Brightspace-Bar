...
private struct SourceCase: Sendable, CustomStringConvertible {
    ...
    /// Concern 3.5 replaces this closure body with `BrightspaceCourseSource(...)`.
    ///
    /// It is a throwing placeholder rather than a reference to the real type on purpose:
    /// naming a type that does not exist yet would keep the whole package from compiling,
    /// so polling could never be independently green. Swapping in the real source later
    /// touches this one closure and nothing else — no assertion is rewritten.
    static let live = SourceCase(name: "BrightspaceCourseSource") {
        try BrightspaceCourseSource()
    }
}
