...
/// The real `AssignmentSource`: cookie + CSRF token → JWT → `dropbox/folders` → `[Assignment]`.
///
/// Structurally a sibling of `BrightspaceCourseSource`, on purpose — same three
/// steps, same seam, same taxonomy — so the two network adapters can be read and
/// audited as one shape:
///
///   - `decodeMint(status:body:)` and `decodeAPI(status:)` are pure functions on
///     plain values. They own the failure classification; the network methods just
///     execute a request and hand the raw `(status, bytes)` over. That is what
///     makes the taxonomy inspectable without a socket.
///   - `fetchAssignments` is the impureim sandwich: effect (session) → effect
///     (mint) → effect (GET) → pure (`AssignmentParser.parse`). Parsing, including
///     its stub/malformed taxonomy, is reused verbatim rather than reimplemented.
///
/// SEAM: credentials come from a `SessionProviding`, asked **per fetch** — never
/// held. A session refreshed on disk (or, later, re-minted by a login window) is
/// picked up on the next fetch without restarting anything.
///
/// Secrets discipline: the cookie, the CSRF token, and the JWT never touch a log, a
/// print, an error message, or the disk. Errors carry status codes and generic
/// reasons only.
///
/// One measured constraint worth knowing: experiment 6 proved that once a course's
/// `Access` window closes, every content route answers **403**. That is not an
/// exceptional case to be defended against — it is the expected steady state for
/// last term's courses, and it arrives here as `.httpStatus(403)`, which the store
/// folds into `.preservedStale`.
public struct BrightspaceAssignmentSource: AssignmentSource {
    ...
    public func fetchAssignments(courseId: Int) async throws -> [Assignment] {
        ...
        let body = try await self.getFolders(courseId: courseId, jwt: jwt, credentials) // effect
        return try AssignmentParser.parse(body, courseId: courseId)              // pure
    }

    /// Step 0: ask the seam for credentials. A provider failure is a fetch failure
    /// like any other — mapped into the `CourseSourceError` taxonomy so the store
    /// answers it with `.preservedStale` and the submenu keeps its assignments.
    ...
    ///
    /// The one non-GET request this codebase makes anywhere.
    ...
    /// Step 2: bearer-authenticated GET of one course's assignment folders.
    private func getFolders(
        courseId: Int,
        jwt: String,
        _ credentials: BrightspaceSession
    ) async throws -> Data {
        let path = "/d2l/api/le/\(Self.leVersion)/\(courseId)/dropbox/folders/"
    ...
    /// returning an HTTP response — offline, DNS, timeout, TLS — becomes
    /// `.transport`. Typed errors thrown downstream are passed through untouched.
    ...
    /// status. 403 is the one to expect — it is what an ended course returns. The
    /// body is left to the parser.
