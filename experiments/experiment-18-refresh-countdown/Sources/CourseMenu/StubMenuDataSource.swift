...
public struct StubMenuDataSource: MenuDataSource {
    ...
    public static let seeded = MenuModel(rows: [
        ...
        // the whole behaviour arc for free: "Updated just now" ages into
        // "Updated N minutes ago", the countdown counts down, and past the
        // deadline it clamps to "Refreshes soon" — all with no backend.
        .status(.updated(Date())),
