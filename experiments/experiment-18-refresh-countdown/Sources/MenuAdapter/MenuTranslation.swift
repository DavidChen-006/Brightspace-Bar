...
public enum MenuTranslation {
    ...
    ///
    /// `now` is deliberately no longer read: status rows carry the *dates*
    /// (`StatusStamp`) and the GUI formats them against its own fresh `now` at
    /// menu-open. The parameter stays because production's translation genuinely
    /// needs it (course visibility, due dates) and the seam should match. A
    /// consequence worth the whole experiment: the model is now time-invariant,
    /// so `Equatable` finally skips rebuilds across timer ticks with unchanged
    /// data (pinned by `modelIsTimeInvariant`).
    ...
    // formats them when the menu actually opens.
}
