...
/// The real 27-course payload, reached across the package boundary.
///
/// It belongs to experiment 4 and is not a resource of this package, so it is
/// located by walking up from `#filePath` — the same technique experiment 4's own
/// `Fixture` helper uses, and one that needs no `resources:` entry in either
/// `Package.swift`.
enum CrossPackageFixture {
    /// `<PaperShelf>/experiment-4-course-pipeline/Tests/CoursePipelineTests/Fixtures/`
    static var directory: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // Tests/MenuAdapterTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // experiment-5-menubar
            .deletingLastPathComponent()   // <PaperShelf>
            .appending(path: "experiment-4-course-pipeline")
            .appending(path: "Tests")
            .appending(path: "CoursePipelineTests")
